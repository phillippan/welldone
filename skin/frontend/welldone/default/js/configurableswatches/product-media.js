...
var ConfigurableMediaImages = {
    ...
    updateImage: function(el) {
        ...
        var mainImage=$j('#bigImg');
        var zoomConfig = {gallery: "smallGallery"};
        if(targetImage[0].complete) {
            $j('.zoomContainer').remove();
            mainImage.removeData('elevateZoom').attr({"src":swatchImageUrl}).data('zoom-image', swatchImageUrl).elevateZoom(zoomConfig);
        } else {
            mainImage.attr({"src":swatchImageUrl,'data-zoom-image':swatchImageUrl});
    ...
    swapListImage: function(productId, imageObject) {
        ...
        if(imageObject[0].complete) { //swap image immediately
            ...
            par=originalImage.parent();
            jQuery('.img-second',par).remove();
